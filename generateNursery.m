...
% use for error statistics 
true_tree_location = zeros((M*K),3);
...
index = 1; % need to initialize index to append valus in 'true_tree_location'
...
for j=1:K
    for i=1:M
        ...
        if rand > 0.002 
            ...
            % append values into variable 
            true_tree_location(index,:) = [x(i,j), y(i,j), radius*(Xmax/C)];
            index = index +1;
        end
    end
end
...
set(gca,'YDir','normal');
axis equal
title('2-D Ground Truth Nursery ')
